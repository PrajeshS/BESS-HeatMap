...
p_max = st.sidebar.number_input('Max Power to Test (MW)', 50, 200, 100)

# Industry Standard Durations
durations = [0.25, 0.5, 1.0, 2.0, 4.0] 
powers = np.arange(10, p_max + 10, 10)
...
with st.spinner('Generating Industry-Standard Sensitivity Matrix...'):
    ...
    df = pd.DataFrame(matrix, index=powers, columns=[f"{d}h ({1/d if d!=0 else 0:.1f}C)" for d in durations])
...
st.success("✅ Matrix generated using Numba JIT engine. Ready for deployment.")
